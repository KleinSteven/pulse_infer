...
#include "pulse/ops/kernels/add_kernel.hpp"
...
[[nodiscard]] Result<void> add(const Tensor& input1, const Tensor& input2, Tensor& output) {
    ...
    if (input1.device() == DeviceType::CPU) {
        if (input1.dtype() == DataType::Float32) {
            ...
            kernels::add<f32>(input1_span, input2_span, output_span, size);
            ...
            kernels::add<f64>(input1_span, input2_span, output_span, size);
    ...
    return Err<void>(ErrorCode::NotImplemented, "Add operation not implemented for this device");
}
