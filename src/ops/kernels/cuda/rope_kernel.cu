...
template<typename T>
__global__ void rope_positions_kernel(const T* input,
                                      T* output,
                                      const i32* positions,
                                      i32 batch_size,
                                      i32 rows_per_batch,
                                      ...
                                      float theta) {
    const i64 total =
        static_cast<i64>(batch_size) * static_cast<i64>(rows_per_batch) * static_cast<i64>(head_dim);
    const i64 idx = static_cast<i64>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (idx >= total) {
        return;
    }
    ...
    const i32 dim = static_cast<i32>(idx % head_dim);
    const i64 row_idx = idx / head_dim;
    const i32 batch_idx = static_cast<i32>(row_idx / rows_per_batch);
    const i32 pos = positions[batch_idx];
    ...
    const float exponent = static_cast<float>(pair_base_dim) / static_cast<float>(rotary_dim);
    const float angle = static_cast<float>(pos) / powf(theta, exponent);
    const float cos_value = cosf(angle);
    const float sin_value = sinf(angle);
    const float rotated =
        (dim & 1) == 0 ? (x0 * cos_value - x1 * sin_value) : (x0 * sin_value + x1 * cos_value);
...
__global__ void rope_positions_f64_kernel(const f64* input,
                                          f64* output,
                                          const i32* positions,
                                          i32 batch_size,
                                          i32 rows_per_batch,
    ...
    const double x1 = input[pair_base_idx + 1];
    const double exponent = static_cast<double>(pair_base_dim) / static_cast<double>(rotary_dim);
    const double angle = static_cast<double>(pos) / pow(theta, exponent);
    const double cos_value = cos(angle);
    const double sin_value = sin(angle);
    output[idx] =
        (dim & 1) == 0 ? (x0 * cos_value - x1 * sin_value) : (x0 * sin_value + x1 * cos_value);
...
Result<void> rope_positions_cuda_launch(const void* input,
                                        ...
                                        DataType dtype,
                                        cudaStream_t stream) {
    if (batch_size <= 0 || rows_per_batch <= 0 || head_dim <= 0) {
        return Ok();
    }
    ...
    switch (dtype) {
        case DataType::Float16:
            LAUNCH_KERNEL(rope_positions_kernel<f16>,
                          block_num,
                          thread_num,
                          stream,
                          static_cast<const f16*>(input),
            ...
            break;
        case DataType::BFloat16:
            LAUNCH_KERNEL(rope_positions_kernel<bf16>,
                          block_num,
                          thread_num,
                          stream,
                          static_cast<const bf16*>(input),
            ...
            break;
        case DataType::Float32:
            LAUNCH_KERNEL(rope_positions_kernel<f32>,
                          block_num,
                          thread_num,
                          stream,
                          static_cast<const f32*>(input),
            ...
            break;
        case DataType::Float64:
            LAUNCH_KERNEL(rope_positions_f64_kernel,
                          block_num,
                          thread_num,
                          stream,
                          static_cast<const f64*>(input),
        ...
        default:
            return Err<void>(ErrorCode::NotImplemented,
                             "CUDA RoPE positions only supports Float16/BFloat16/Float32/Float64");
    }

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        pulse::error("CUDA rope positions kernel launch failed for {}: {}",
                     data_type_str(dtype),
                     cudaGetErrorString(err));
        return Err<void>(ErrorCode::CudaError,
                         std::format("CUDA rope positions kernel launch failed for {}: {}",
                                     data_type_str(dtype),
                                     cudaGetErrorString(err)));
    }
